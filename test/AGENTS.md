...
- Focused file: `bunx vitest run test/path/to/file.test.ts`
- Focused case: `bunx vitest run test/path/to/file.test.ts -t "case name"`
- Full suite: `bun run test`
- Run `bun run build` before focused CLI tests when implementation changes may leave `dist/` stale.
