...
describe('InitCommand', () => {
  ...
  beforeEach(async () => {
    testDir = path.join(os.tmpdir(), `spok-init-test-${Date.now()}`);
    await fs.mkdir(testDir, { recursive: true });
    originalEnv = { ...process.env };
    // Use a temp dir for global config to avoid reading real config
    configTempDir = path.join(os.tmpdir(), `spok-config-init-${Date.now()}`);
  ...
  describe('execute with --tools flag', () => {
    it('should create Spok directory structure', async () => {
      const initCommand = new InitCommand({ tools: 'claude', force: true });

      await initCommand.execute(testDir);

      const spokPath = path.join(testDir, 'spok');
      expect(await directoryExists(spokPath)).toBe(true);
      expect(await directoryExists(path.join(spokPath, 'specs'))).toBe(true);
      expect(await directoryExists(path.join(spokPath, 'changes'))).toBe(true);
      expect(await directoryExists(path.join(spokPath, 'changes', 'archive'))).toBe(true);
    });
    ...
    it('should create config.yaml with default schema', async () => {
      ...
      const configPath = path.join(testDir, 'spok', 'config.yaml');
    ...
    it('should create skills in Cursor skills directory', async () => {
      ...
      const skillFile = path.join(testDir, '.cursor', 'skills', 'spok-explore', 'SKILL.md');
      expect(await fileExists(skillFile)).toBe(true);
    });
    ...
    it('should create skills in Windsurf skills directory', async () => {
      ...
      const skillFile = path.join(testDir, '.windsurf', 'skills', 'spok-explore', 'SKILL.md');
      expect(await fileExists(skillFile)).toBe(true);
    });
    ...
    it('should support Kimi CLI as an adapterless skills-only tool', async () => {
      ...
      const skillFile = path.join(testDir, '.kimi', 'skills', 'spok-explore', 'SKILL.md');
      expect(await fileExists(skillFile)).toBe(true);
    ...
    it('should create skills for multiple tools at once', async () => {
      ...
      const claudeSkill = path.join(testDir, '.claude', 'skills', 'spok-explore', 'SKILL.md');
      const cursorSkill = path.join(testDir, '.cursor', 'skills', 'spok-explore', 'SKILL.md');
    ...
    it('should select all tools with --tools all option', async () => {
      ...
      const claudeSkill = path.join(testDir, '.claude', 'skills', 'spok-explore', 'SKILL.md');
      const cursorSkill = path.join(testDir, '.cursor', 'skills', 'spok-explore', 'SKILL.md');
      const windsurfSkill = path.join(testDir, '.windsurf', 'skills', 'spok-explore', 'SKILL.md');
    ...
    it('should skip tool configuration with --tools none option', async () => {
      ...
      // Should create Spok structure but no skills
      const spokPath = path.join(testDir, 'spok');
      expect(await directoryExists(spokPath)).toBe(true);
    ...
    it('should not create config.yaml if it already exists', async () => {
      ...
      const configPath = path.join(spokDir, 'config.yaml');
    ...
    it('should handle non-existent target directory', async () => {
      ...
      const spokPath = path.join(newDir, 'spok');
      expect(await directoryExists(spokPath)).toBe(true);
    });
  ...
  describe('skill content validation', () => {
    it('should generate valid SKILL.md with YAML frontmatter', async () => {
      ...
      expect(content).toContain('name: spok-explore');
      expect(content).toContain('description:');
    ...
    it('should include propose skill instructions', async () => {
      ...
      const skillFile = path.join(testDir, '.claude', 'skills', 'spok-propose', 'SKILL.md');
      const content = await fs.readFile(skillFile, 'utf-8');

      expect(content).toContain('name: spok-propose');
    });
    ...
    it('should include apply-change skill instructions', async () => {
      ...
      const skillFile = path.join(testDir, '.claude', 'skills', 'spok-apply-change', 'SKILL.md');
      const content = await fs.readFile(skillFile, 'utf-8');

      expect(content).toContain('name: spok-apply-change');
    });
...
describe('InitCommand - profile and detection features', () => {
  ...
  beforeEach(async () => {
    testDir = path.join(os.tmpdir(), `spok-init-profile-test-${Date.now()}`);
    await fs.mkdir(testDir, { recursive: true });
    originalEnv = { ...process.env };
    // Use a temp dir for global config to avoid polluting real config
    configTempDir = path.join(os.tmpdir(), `spok-config-test-${Date.now()}`);
    await fs.mkdir(configTempDir, { recursive: true });
    process.env.XDG_CONFIG_HOME = configTempDir;
    vi.spyOn(console, 'log').mockImplementation(() => {});
    confirmMock.mockReset();
  ...
  it('should use --profile flag to override global config', async () => {
    ...
    const proposeSkill = path.join(testDir, '.claude', 'skills', 'spok-propose', 'SKILL.md');
    expect(await fileExists(proposeSkill)).toBe(true);

    // Non-core skills (from the custom profile) should NOT be created
    const newChangeSkill = path.join(testDir, '.claude', 'skills', 'spok-new-change', 'SKILL.md');
  ...
  it('should preselect configured tools but not directory-detected tools in extend mode', async () => {
    // Simulate existing Spok project (extend mode).
    await fs.mkdir(path.join(testDir, 'spok'), { recursive: true });

    // Configured with Spok
    ...
    // Directory detected only (not configured with Spok)
  ...
  it('should preselect detected tools for first-time interactive setup', async () => {
    // First-time init: no spok/ directory and no configured Spok skills.
    await fs.mkdir(path.join(testDir, '.github'), { recursive: true });
    await fs.writeFile(path.join(testDir, '.github', 'copilot-instructions.md'), '');
  ...
  it('should respect custom profile from global config', async () => {
    ...
    const exploreSkill = path.join(testDir, '.claude', 'skills', 'spok-explore', 'SKILL.md');
    const newChangeSkill = path.join(testDir, '.claude', 'skills', 'spok-new-change', 'SKILL.md');
