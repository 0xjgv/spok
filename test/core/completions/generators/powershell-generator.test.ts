...
describe('PowerShellGenerator', () => {
	...
	describe('generate', () => {
		it('should generate valid PowerShell completion script with header', () => {
			...
			expect(script).toContain('# PowerShell completion script for Spok CLI');
			expect(script).toContain('$spokCompleter = {');
			expect(script).toContain('Register-ArgumentCompleter');
		});

		it('should register argument completer for spok command', () => {
			const commands: CommandDefinition[] = [
				{
					name: 'init',
					description: 'Initialize Spok',
			...
			expect(script).toContain('Register-ArgumentCompleter -CommandName spok');
			expect(script).toContain('-ScriptBlock $spokCompleter');
		});
		...
		it('should handle positional arguments for change-id', () => {
			...
			expect(script).toContain('Get-SpokChanges');
		});
		...
		it('should handle positional arguments for spec-id', () => {
			...
			expect(script).toContain('Get-SpokSpecs');
		});
		...
		it('should handle positional arguments for schema names', () => {
			...
			expect(script).toContain('Get-SpokSchemas');
			expect(script).toContain('spok __complete schemas 2>$null');
		});
		...
		it('should generate dynamic completion helper for changes', () => {
			...
			expect(script).toContain('function Get-SpokChanges');
			expect(script).toContain('spok __complete changes 2>$null');
		...
		it('should generate dynamic completion helper for specs', () => {
			...
			expect(script).toContain('function Get-SpokSpecs');
			expect(script).toContain('spok __complete specs 2>$null');
		});
