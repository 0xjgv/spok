...
describe('migration', () => {
  ...
  beforeEach(async () => {
    projectDir = path.join(os.tmpdir(), `spok-migration-project-${randomUUID()}`);
    configHome = path.join(os.tmpdir(), `spok-migration-config-${randomUUID()}`);
  ...
  it('migrates to custom skills delivery when only managed skills are detected', async () => {
    await writeSkill(projectDir, 'spok-explore');
    await writeSkill(projectDir, 'spok-apply-change');
