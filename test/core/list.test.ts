...
describe('ListCommand', () => {
  ...
  beforeEach(async () => {
    ...
    tempDir = path.join(os.tmpdir(), `spok-list-test-${Date.now()}`);
  ...
  describe('execute', () => {
    it('should handle missing spok/changes directory', async () => {
      const listCommand = new ListCommand();
      
      await expect(listCommand.execute(tempDir, 'changes')).rejects.toThrow(
        "No Spok changes directory found. Run 'spok init' first."
      );
    });
