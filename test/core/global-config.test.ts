...
describe('global-config', () => {
  ...
  beforeEach(() => {
    ...
    tempDir = path.join(os.tmpdir(), `spok-global-config-test-${Date.now()}`);
  ...
  describe('constants', () => {
    it('should export correct directory name', () => {
      expect(GLOBAL_CONFIG_DIR_NAME).toBe('spok');
    });
  ...
  describe('getGlobalConfigDir', () => {
    it('should use XDG_CONFIG_HOME when set', () => {
      ...
      expect(result).toBe(path.join(tempDir, 'spok'));
    });
    ...
    it('should fall back to ~/.config on Unix/macOS without XDG_CONFIG_HOME', () => {
      ...
      // On non-Windows, should use ~/.config/spok
      if (os.platform() !== 'win32') {
        expect(result).toBe(path.join(os.homedir(), '.config', 'spok'));
      }
    });
    ...
    it('should use APPDATA on Windows when XDG_CONFIG_HOME is not set', () => {
      ...
      if (os.platform() === 'win32') {
        ...
        if (appData) {
          const result = getGlobalConfigDir();
          expect(result).toBe(path.join(appData, 'spok'));
        }
      }
    });
  });
  ...
  describe('getGlobalConfigPath', () => {
    it('should return path to config.json in config directory', () => {
      ...
      expect(result).toBe(path.join(tempDir, 'spok', 'config.json'));
    });
  });
  ...
  describe('getGlobalDataDir', () => {
    it('should use POSIX separators for Unix-like platform overrides', () => {
      ...
      ).toBe('/home/tabish/.local/share/spok');
    ...
    it('should use Windows separators for native Windows platform overrides', () => {
      ...
      ).toBe('C:\\Users\\Tabish\\AppData\\Local\\spok');
      ...
      ).toBe('D:\\Users\\Tabish\\AppData\\Local\\spok');
    });
  });
  ...
  describe('getGlobalConfig', () => {
    ...
    describe('schema evolution', () => {
      ...
      it('should preserve explicit delivery value from config', () => {
        process.env.XDG_CONFIG_HOME = tempDir;
        const configDir = path.join(tempDir, 'spok');
        const configPath = path.join(configDir, 'config.json');
      ...
      it('should round-trip delivery field correctly', () => {
        process.env.XDG_CONFIG_HOME = tempDir;
  ...
  describe('saveGlobalConfig', () => {
    ...
    it('should write config to file', () => {
      process.env.XDG_CONFIG_HOME = tempDir;
      const configPath = path.join(tempDir, 'spok', 'config.json');
