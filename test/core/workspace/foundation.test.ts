...
describe('workspace foundation', () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'spok-workspace-foundation-'));
...
`
  ...
  describe('path helpers', () => {
    it('exposes the workspace constants', () => {
      expect(WORKSPACE_METADATA_DIR_NAME).toBe('.spok-workspace');
    ...
    it('returns workspace file paths using platform-aware path helpers', () => {
      ...
      expect(getWorkspaceMetadataDir(workspaceRoot)).toBe(
        path.join(workspaceRoot, '.spok-workspace')
      );
      ...
      expect(getWorkspaceLocalStatePath(workspaceRoot)).toBe(
        path.join(workspaceRoot, '.spok-workspace', 'local.yaml')
      );
    ...
    it('preserves Windows-style location strings when building workspace file paths', () => {
      ...
      expect(getWorkspaceSharedStatePath(workspaceRoot)).toBe(
        'D:\\repos\\platform-workspace\\.spok-workspace\\workspace.yaml'
      );
      expect(getWorkspaceLocalStatePath(workspaceRoot)).toBe(
        'D:\\repos\\platform-workspace\\.spok-workspace\\local.yaml'
      );
    });
    ...
    it('uses getGlobalDataDir for managed workspace and registry locations', () => {
      ...
      expect(getManagedWorkspacesDir()).toBe(path.join(tempDir, 'spok', 'workspaces'));
      expect(getManagedWorkspaceRoot('platform')).toBe(
        path.join(tempDir, 'spok', 'workspaces', 'platform')
      );
      expect(getWorkspaceRegistryPath()).toBe(
        path.join(tempDir, 'spok', 'workspaces', 'registry.yaml')
      );
    });
    ...
    it('uses the Linux data-dir fallback under the managed workspaces directory', () => {
      ...
      expect(getManagedWorkspacesDir({ globalDataDir: dataDir })).toBe(
        '/home/tabish/.local/share/spok/workspaces'
      );
    });
    ...
    it('uses the native Windows data-dir fallback under the managed workspaces directory', () => {
      ...
      expect(getManagedWorkspacesDir({ globalDataDir: dataDir })).toBe(
        'C:\\Users\\Tabish\\AppData\\Local\\spok\\workspaces'
      );
    });

    it('exposes the portable collaboration ignore rule for local state', () => {
      expect(WORKSPACE_LOCAL_STATE_IGNORE_PATTERN).toBe('.spok-workspace/local.yaml');
      expect(getWorkspacePortableIgnorePatterns()).toEqual(['.spok-workspace/local.yaml']);
  ...
  describe('workspace folder detection', () => {
    ...
    it('does not mistake repo-local spok projects for coordination workspaces', async () => {
      const repoRoot = path.join(tempDir, 'repo');
      fs.mkdirSync(path.join(repoRoot, 'spok', 'changes', 'add-feature'), {
        recursive: true,
      });
      fs.mkdirSync(path.join(repoRoot, 'spok', 'specs'), { recursive: true });

      await expect(findWorkspaceRoot(path.join(repoRoot, 'spok', 'changes'))).resolves.toBe(
        null
      );
    });

    it('detects a workspace even when a linked path has no repo-local spok state', async () => {
      const workspaceRoot = createWorkspaceRoot();
...
`;
      ...
      expect(refreshed.match(/SPOK:WORKSPACE-GUIDANCE:START/gu)).toHaveLength(1);
      expect(applyWorkspaceGuidanceBlock('# Team Notes\n')).toContain(
        '<!-- SPOK:WORKSPACE-GUIDANCE:START -->'
      );
    });
...
`);
    ...
    it('reads the local registry from the standard registry path', async () => {
      const globalDataDir = path.join(tempDir, 'data', 'spok');
