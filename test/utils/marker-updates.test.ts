...
describe('FileSystemUtils.updateFileWithMarkers', () => {
  let testDir: string;
  const START_MARKER = '<!-- SPOK:START -->';
  const END_MARKER = '<!-- SPOK:END -->';

  beforeEach(async () => {
    testDir = path.join(os.tmpdir(), `spok-marker-test-${Date.now()}`);
...
After block content`;
  ...
  describe('shell markers', () => {
    const SHELL_START = '# SPOK:START';
    const SHELL_END = '# SPOK:END';
...
export EDITOR="vim"`;
      ...
      expect(result).not.toContain('alias spok');
