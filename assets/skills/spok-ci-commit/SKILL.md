...
3. **Execute upon confirmation:**
   ...
   - Never stage a path you did not modify — other agents may be working in this repository concurrently, and their in-progress edits are not yours to commit
   - Never treat a gitignored path as committable — run `git check-ignore` on a path before listing it as an expected file, and drop the ones that are ignored
