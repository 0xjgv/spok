...
This guide helps you transition from the legacy Spok workflow to OPSX. The migration is designed to be smooth—your existing work is preserved, and the new system offers more flexibility.
...
| Aspect | Legacy | OPSX |
|--------|--------|------|
| **Commands** | `/spok:proposal`, `/spok:apply`, `/spok:archive` | Default: `/opsx:propose`, `/opsx:apply`, `/opsx:sync`, `/opsx:archive` (expanded workflow commands optional) |
| **Workflow** | Create all artifacts at once | Create incrementally or all at once—your choice |
| **Going back** | Awkward phase gates | Natural—update any artifact anytime |
| **Customization** | Fixed structure | Schema-driven, fully hackable |
| **Configuration** | `CLAUDE.md` with markers + `project.md` | Clean config in `spok/config.yaml` |
...
- **Active changes in `spok/changes/`** — Completely preserved. You can continue them with OPSX commands.
- **Archived changes** — Untouched. Your history remains intact.
- **Main specs in `spok/specs/`** — Untouched. These are your source of truth.
- **Your content in CLAUDE.md, AGENTS.md, etc.** — Preserved. Only the Spok marker blocks are removed; everything you wrote stays.

### What Gets Removed

Only Spok-managed files that are being replaced:

| What | Why |
|------|-----|
| Legacy slash command directories/files | Replaced by the new skills system |
| `spok/AGENTS.md` | Obsolete workflow trigger |
| Spok markers in `CLAUDE.md`, `AGENTS.md`, etc. | No longer needed |

**Legacy command locations by tool** (examples—your tool may vary):

- Claude Code: `.claude/commands/spok/`
- Cursor: `.cursor/commands/spok-*.md`
- Windsurf: `.windsurf/workflows/spok-*.md`
- Cline: `.clinerules/workflows/spok-*.md`
- Roo: `.roo/commands/spok-*.md`
- GitHub Copilot: `.github/prompts/spok-*.prompt.md` (IDE extensions only; not supported in Copilot CLI)
...
The removal list may seem long, but these are all files that Spok originally created. Your own content is never deleted.

### What Needs Your Attention

One file requires manual migration:

**`spok/project.md`** — This file isn't deleted automatically because it may contain project context you've written. You'll need to:

1. Review its contents
2. Move useful context to `spok/config.yaml` (see guidance below)
...
The new `config.yaml` context is **actively injected into every Spok planning request**. This means your project conventions, tech stack, and rules are always present when the AI is creating artifacts. Higher reliability.
...
Both `spok init` and `spok update` detect legacy files and guide you through the same cleanup process. Use whichever fits your situation:
...
```
Upgrading to the new Spok

Spok now uses agent skills, the emerging standard across coding
agents. This simplifies your setup while keeping everything working
as before.

Files to remove
No user content to preserve:
  • .claude/commands/spok/
  • spok/AGENTS.md

Files to update
Spok markers will be removed, your content preserved:
  • CLAUDE.md
  • AGENTS.md
...
Needs your attention
  • spok/project.md
    ...
    The new spok/config.yaml has a "context:" section for planning
    context. This is included in every Spok request and works more
...
2. Spok markers are stripped from `CLAUDE.md`, `AGENTS.md`, etc. (your content stays)
3. `spok/AGENTS.md` is deleted
4. New skills are installed in `.claude/skills/`
5. `spok/config.yaml` is created with a default schema
...
The old `spok/project.md` was a freeform markdown file for project context. The new `spok/config.yaml` is structured and—critically—**injected into every planning request** so your conventions are always present when the AI works.
...
```
I'm migrating from Spok's old project.md to the new config.yaml format.
...
Enable expanded commands with `spok config profile`, then run `spok update`.

### Command Mapping from Legacy

| Legacy | OPSX Equivalent |
|--------|-----------------|
| `/spok:proposal` | `/opsx:propose` (default) or `/opsx:new` then `/opsx:ff` (expanded) |
| `/spok:apply` | `/opsx:apply` |
| `/spok:archive` | `/opsx:archive` |
...
```
.claude/skills/
├── spok-explore/SKILL.md
├── spok-new-change/SKILL.md
├── spok-continue-change/SKILL.md
├── spok-apply-change/SKILL.md
└── ...
```
...
```bash
spok status --change add-my-feature
```
...
2. **Change metadata**: `.spok.yaml` in the change directory
...
1. Ensure the file is at `spok/config.yaml` (not `.yml`)
...
│       ├── spok-propose/     # default core profile
│       ├── spok-explore/
│       ├── spok-apply-change/
│       ├── spok-sync-specs/
│       └── ...                   # expanded profile adds new/continue/ff/etc.
├── CLAUDE.md                     # Spok markers removed, your content preserved
└── AGENTS.md                     # Spok markers removed, your content preserved
```

### What's Gone

- `.claude/commands/spok/` — replaced by `.claude/skills/`
- `spok/AGENTS.md` — obsolete
- `spok/project.md` — migrate to `config.yaml`, then delete
- Spok marker blocks in `CLAUDE.md`, `AGENTS.md`, etc.
