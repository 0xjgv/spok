...
OPSX is now the standard workflow for Spok.

It's a **fluid, iterative workflow** for Spok changes. No more rigid phases — just actions you can take anytime.

## Why This Exists

The legacy Spok workflow works, but it's **locked down**:
...
- **Spok contributors** — experiment with new approaches without releases
...
```bash
# Make sure you have spok installed — skills are automatically generated
spok init
```

This creates skills in `.claude/skills/` (or equivalent) that AI coding assistants auto-detect.

By default, Spok uses the `core` workflow profile (`propose`, `explore`, `apply`, `sync`, `archive`). If you want the expanded workflow commands (`new`, `continue`, `ff`, `verify`, `bulk-archive`, `onboard`), configure them with `spok config profile` and apply with `spok update`.

During setup, you'll be prompted to create a **project config** (`spok/config.yaml`). This is optional but recommended.
...
Config is created during `spok init`, or manually:
...
2. Change metadata (`.spok.yaml` in change directory)
...
- Run `spok schemas --json` to see artifact IDs for each schema

**Config not being applied:**
- Ensure file is at `spok/config.yaml` (not `.yml`)
...
| | Legacy (`/spok:proposal`) | OPSX (`/opsx:*`) |
|---|---|---|
...
│         │                     │                     │                       │
│         ▼                     ▼                     ▼                       │
│   /spok:proposal   /spok:apply      /spok:archive              │
│                                                                             │
...
│                    │                                                        │
│                    ▼                                                        │
│   Generated Command Files (.claude/commands/spok/*.md)                  │
│                                                                             │
...
│                    │                                                        │
│                    ▼                                                        │
│   Skill Files (.claude/skills/spok-*/SKILL.md)                          │
│                                                                             │
...
```
  ...
  │  ┌────────────────────────────────────────────────────────────────────┐  │
  │  │  $ spok status --change "add-auth" --json                      │  │
  │  │                                                                    │  │
  │  │  {                                                                 │  │
  │  │    "artifacts": [                                                  │  │
  ...
  │  ┌────────────────────────────────────────────────────────────────────┐  │
  │  │  $ spok instructions specs --change "add-auth" --json          │  │
  │  │                                                                    │  │
  │  │  {                                                                 │  │
...
Schemas are stored in `spok/schemas/` (project-local, version controlled) or `~/.local/share/spok/schemas/` (user global).

**Schema structure:**
```
spok/schemas/research-first/
├── schema.yaml
└── templates/
    ├── research.md
    ├── proposal.md
...
- Check status anytime: `spok status --change "name"`

## Feedback

This is rough. That's intentional — we're learning what works.

Found a bug? Have ideas? Join us on [Discord](https://discord.gg/YctCnvvshC) or open an issue on [GitHub](https://github.com/Fission-AI/spok/issues).
