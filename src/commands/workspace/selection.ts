...
export async function selectWorkspaceRootForCommand(
  ...
  if (!currentWorkspaceRoot) {
    throw new WorkspaceCliError(
      `No Spok workspace found at '${workspaceRoot}'.`,
      'workspace_not_found',
      {
        target: 'workspace.root',
        fix: 'Pass a path inside an Spok workspace.',
      }
    );
  }
...
export async function selectWorkspaceForCommand(
  ...
  if (options.workspace) {
    ...
    if (!registryRoot) {
      throw new WorkspaceCliError(
        `Unknown Spok workspace '${workspaceName}'.`,
        'workspace_not_found',
        {
          target: 'workspace.name',
          fix: 'Run spok workspace list to see known workspaces.',
        }
      );
    }
  ...
  if (entries.length === 0) {
    throw new WorkspaceCliError(
      "No known Spok workspaces. Run 'spok workspace setup' first.\nAfter at least one workspace is known locally, you can also pass --workspace <name>.",
      'no_known_workspaces',
      {
        target: 'workspace.name',
        fix: 'spok workspace setup',
      }
    );
  }
  ...
  if (options.json || resolveNoInteractive(options) || !isInteractive(options)) {
    ...
    const fix = usesPositionalName
      ? `spok workspace ${commandName} <name>`
      : `spok workspace ${commandName} --workspace <name>`;

    throw new WorkspaceCliError(
      usesPositionalName
        ? `Multiple Spok workspaces are known. Known workspaces: ${knownNames}. Pass a workspace name.`
        : `Multiple Spok workspaces are known. Known workspaces: ${knownNames}. Pass --workspace <name>.`,
