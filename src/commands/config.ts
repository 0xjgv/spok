...
import { SPOK_DIR_NAME } from '../core/config.js';
...
const WORKFLOW_PROMPT_META: Record<string, WorkflowPromptMeta> = {
  ...
  onboard: {
    name: 'Onboard',
    description: 'Guided onboarding flow for Spok',
  },
};
...
function maybeWarnProjectConfigDrift(
  ...
  const spokDir = path.join(projectDir, SPOK_DIR_NAME);
  if (!fs.existsSync(spokDir)) {
    return;
  }
  if (!hasProjectConfigDrift(projectDir, state.workflows, state.delivery)) {
    return;
  }
  console.log(colorize('Warning: Global config is not applied to this project. Run `spok update` to sync.'));
}
...
async function maybeWarnConfigDrift(
  ...
  if (workspaceContext) {
    ...
    if (hasWorkspaceSkillProfileDrift(localState)) {
      console.log(
        colorize(
          'Warning: Workspace-local agent skills are out of sync with the active global profile. Run `spok workspace update` to sync.'
        )
      );
    }
    return;
...
function printConfigProfileApplyGuidance(workspaceContext: WorkspaceConfigProfileContext | null): void {
  if (workspaceContext) {
    console.log('Config updated. Run `spok workspace update` to apply it to workspace-local skills.');
    return;
  }

  console.log('Config updated. Run `spok update` in your projects to apply.');
}
...
export function registerConfigCommand(program: Command): void {
  const configCmd = program
    .command('config')
    .description('View and modify global Spok configuration')
  ...
  configCmd
    ...
    .action((key: string, value: string, options: { string?: boolean; allowUnknown?: boolean }) => {
      ...
      if (!keyValidation.valid && !allowUnknown) {
        ...
        console.error('Use "spok config list" to see available keys.');
  ...
  configCmd
    ...
    .action(async (options: { all?: boolean; yes?: boolean }) => {
      if (!options.all) {
        ...
        console.error('Usage: spok config reset --all [-y]');
        process.exitCode = 1;
        return;
      }
  ...
  configCmd
    ...
    .action(async (preset?: string) => {
      // Preset shortcut: `spok config profile core`
      if (preset === 'core') {
        const config = getGlobalConfig();
        config.profile = 'core';
      ...
      if (!process.stdout.isTTY) {
        console.error('Interactive mode required. Use `spok config profile core` or set config via environment/flags.');
        process.exitCode = 1;
        return;
      }
      ...
      try {
        ...
        if (workspaceContext) {
          ...
          if (applyNow) {
            try {
              execSync('npx spok workspace update', {
                stdio: 'inherit',
                cwd: workspaceContext.commandCwd,
              });
              console.log('Run `spok workspace update` in your other workspaces to apply.');
            } catch {
              console.error('`spok workspace update` failed. Please run it manually to apply the profile changes.');
              process.exitCode = 1;
            }
            return;
          }
        ...
        // Check if inside an Spok project
        ...
        if (fs.existsSync(spokDir)) {
          ...
          if (applyNow) {
            try {
              execSync('npx spok update', { stdio: 'inherit', cwd: projectDir });
              console.log('Run `spok update` in your other projects to apply.');
            } catch {
              console.error('`spok update` failed. Please run it manually to apply the profile changes.');
              process.exitCode = 1;
            }
            return;
          }
