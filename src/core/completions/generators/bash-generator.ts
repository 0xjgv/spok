...
/**
 * Generates Bash completion scripts for the Spok CLI.
...
export class BashGenerator implements CompletionGenerator {
  ...
  generate(commands: CommandDefinition[]): string {
    ...
    return `# Bash completion script for Spok CLI
...
${helpers}
complete -F _spok_completion spok
`;
  }
  ...
  private generatePositionalCompletion(positionalType: string | undefined, indent: string): string[] {
    ...
    switch (positionalType) {
      case 'change-id':
        lines.push(`${indent}_spok_complete_changes`);
        break;
      case 'spec-id':
        lines.push(`${indent}_spok_complete_specs`);
        break;
      case 'change-or-spec-id':
        lines.push(`${indent}_spok_complete_items`);
        break;
      case 'schema-name':
        lines.push(`${indent}_spok_complete_schemas`);
        break;
      case 'shell':
