 ...
 * 2. Schema from .spok.yaml metadata (if exists in change directory)
...
export function loadChangeContext(
  ...
  const changeDir = FileSystemUtils.canonicalizeExistingPath(
    options.changeDir ?? path.join(projectRoot, 'spok', 'changes', changeName)
  );
...
function buildNextSteps(
  ...
  if (readyArtifact) {
    steps.push(
      `Run spok instructions ${readyArtifact.id} --change "${context.changeName}" --json before writing that artifact.`
    );
