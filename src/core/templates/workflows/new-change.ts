...
export function getNewChangeSkillTemplate(): SkillTemplate {
  return {
    name: 'spok-new-change',
    description: 'Start a new Spok change using the experimental artifact workflow. Use when the user wants to create a new feature, fix, or modification with a structured step-by-step approach.',
...
2. **Determine the workflow schema**
   ...
   **Use a different schema only if the user mentions:**
   ...
   - "show workflows" or "what workflows" → run \`spok schemas --json\` and let them choose
...
5. **Get instructions for the first artifact**
   ...
   \`\`\`bash
   spok instructions <first-artifact-id> --change "<name>"
   \`\`\`
