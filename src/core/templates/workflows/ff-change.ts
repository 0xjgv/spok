...
export function getFfChangeSkillTemplate(): SkillTemplate {
  return {
    name: 'spok-ff-change',
    description: 'Fast-forward through Spok artifact creation. Use when the user wants to quickly create all artifacts needed for implementation without stepping through each one individually.',
...
4. **Create artifacts in sequence until apply-ready**
   ...
   b. **Continue until all \`applyRequires\` artifacts are complete**
      - After creating each artifact, re-run \`spok status --change "<name>" --json\`
