...
export class ViewCommand {
  async execute(targetPath: string = '.'): Promise<void> {
    const spokDir = path.join(targetPath, 'spok');
    
    if (!fs.existsSync(spokDir)) {
      console.error(chalk.red('No spok directory found'));
      process.exit(1);
    }
    ...
    const changesData = await this.getChangesData(spokDir);
    const specsData = await this.getSpecsData(spokDir);
    ...
    console.log('\n' + '═'.repeat(60));
    console.log(chalk.dim(`\nUse ${chalk.white('spok list --changes')} or ${chalk.white('spok list --specs')} for detailed views`));
  }

  private async getChangesData(spokDir: string): Promise<{
    draft: Array<{ name: string }>;
    active: Array<{ name: string; progress: { total: number; completed: number } }>;
    completed: Array<{ name: string }>;
  }> {
    const changesDir = path.join(spokDir, 'changes');
  ...
  private async getSpecsData(spokDir: string): Promise<Array<{ name: string; requirementCount: number }>> {
    const specsDir = path.join(spokDir, 'specs');
