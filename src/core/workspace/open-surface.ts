...
export const WORKSPACE_GUIDANCE_START_MARKER = '<!-- SPOK:WORKSPACE-GUIDANCE:START -->';
export const WORKSPACE_GUIDANCE_END_MARKER = '<!-- SPOK:WORKSPACE-GUIDANCE:END -->';

export const WORKSPACE_GUIDANCE_BODY = `# Spok Workspace Guidance

This directory is an Spok workspace for planning across linked repos or folders.
...
- Use Spok workspace commands instead of hand-editing \`.spok-workspace/*.yaml\`.`;
...
export function applyWorkspaceGuidanceBlock(existingContent: string): string {
  ...
  if (startIndex !== -1 || endIndex !== -1) {
    if (startIndex === -1 || endIndex === -1 || endIndex < startIndex) {
      throw new Error('Invalid Spok workspace guidance marker state in AGENTS.md.');
    }
