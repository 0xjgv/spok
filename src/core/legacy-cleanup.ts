/**
 * Legacy cleanup module for detecting and removing Spok artifacts
 ...
 * These were config files created at project root with Spok markers.
 */
export const LEGACY_CONFIG_FILES = [
  ...
  'QODER.md',
  'IFLOW.md',
  'AGENTS.md', // root AGENTS.md (not spok/AGENTS.md)
  'QWEN.md',
] as const;
...
export interface LegacyDetectionResult {
  /** Config files with Spok markers detected */
  configFiles: string[];
  ...
  /** Whether spok/AGENTS.md exists */
  hasOpenspecAgents: boolean;
  /** Whether spok/project.md exists (preserved, migration hint only) */
...
/**
 * Detects all legacy Spok artifacts in a project.
 *
...
/**
 * Detects legacy config files with Spok markers.
 ...
 *
 * @param projectPath - The root path of the project
 * @param pattern - Glob pattern like '.cursor/commands/spok-*.md'
...
/**
 * Detects legacy Spok structure files (AGENTS.md and project.md).
 *
 * @param projectPath - The root path of the project
...
export async function detectLegacyStructureFiles(
  ...
  // Check for spok/AGENTS.md
  const spokAgentsPath = FileSystemUtils.joinPath(projectPath, 'spok', 'AGENTS.md');
  hasOpenspecAgents = await FileSystemUtils.fileExists(spokAgentsPath);

  // Check for spok/project.md (for migration messaging, not deleted)
  const projectMdPath = FileSystemUtils.joinPath(projectPath, 'spok', 'project.md');
  hasProjectMd = await FileSystemUtils.fileExists(projectMdPath);

  // Check for root AGENTS.md with Spok markers
  ...
  if (await FileSystemUtils.fileExists(rootAgentsPath)) {
    ...
    hasRootAgentsWithMarkers = hasSpokMarkers(content);
  }
...
/**
 * Checks if content contains Spok markers.
 *
 * @param content - File content to check
 * @returns True if both start and end markers are present
 */
export function hasSpokMarkers(content: string): boolean {
  return (
    content.includes(SPOK_MARKERS.start) && content.includes(SPOK_MARKERS.end)
  );
}

/**
 * Checks if file content is 100% Spok content (only markers and whitespace outside).
 *
 * @param content - File content to check
 * @returns True if content outside markers is only whitespace
 */
export function isOnlySpokContent(content: string): boolean {
  const startIndex = content.indexOf(SPOK_MARKERS.start);
  const endIndex = content.indexOf(SPOK_MARKERS.end);
  ...
  const after = content.substring(endIndex + SPOK_MARKERS.end.length);

  return before.trim() === '' && after.trim() === '';
}

/**
 * Removes the Spok marker block from file content.
 ...
 *
 * @param content - File content with Spok markers
 * @returns Content with marker block removed
 */
export function removeMarkerBlock(content: string): string {
  return removeMarkerBlockUtil(content, SPOK_MARKERS.start, SPOK_MARKERS.end);
}
...
/**
 * Cleans up legacy Spok artifacts from a project.
 * Preserves spok/project.md (shows migration hint instead of deleting).
 *
 * @param projectPath - The root path of the project
...
export async function cleanupLegacyArtifacts(
  ...
  // Delete spok/AGENTS.md (this is inside spok/, it's Spok-managed)
  if (detection.hasOpenspecAgents) {
    const agentsPath = FileSystemUtils.joinPath(projectPath, 'spok', 'AGENTS.md');
    if (await FileSystemUtils.fileExists(agentsPath)) {
      try {
        await fs.unlink(agentsPath);
        result.deletedFiles.push('spok/AGENTS.md');
      } catch (error: any) {
        result.errors.push(`Failed to delete spok/AGENTS.md: ${error.message}`);
      }
    }
  }

  // Handle root AGENTS.md with Spok markers - remove markers only, NEVER delete
...
export function formatCleanupSummary(result: CleanupResult): string {
  ...
  if (result.deletedFiles.length > 0 || result.deletedDirs.length > 0 || result.modifiedFiles.length > 0) {
    ...
    for (const file of result.modifiedFiles) {
      lines.push(`  ✓ Removed Spok markers from ${file}`);
    }
  }
 ...
 * Only includes Spok-managed files (slash commands, spok/AGENTS.md).
...
function buildRemovalsList(detection: LegacyDetectionResult): Array<{ path: string; explanation: string }> {
  ...
  // Slash command directories (these are 100% Spok-managed)
  ...
  // Slash command files (these are 100% Spok-managed)
...
function buildUpdatesList(detection: LegacyDetectionResult): Array<{ path: string; explanation: string }> {
  ...
  for (const file of detection.configFilesToUpdate) {
    updates.push({ path: file, explanation: 'removing Spok markers' });
  }
...
export function formatDetectionSummary(detection: LegacyDetectionResult): string {
  ...
  lines.push(chalk.bold('Upgrading to the new Spok'));
  lines.push('');
  lines.push('Spok now uses agent skills, the emerging standard across coding');
  ...
  if (updates.length > 0) {
    ...
    lines.push(chalk.dim('Spok markers will be removed, your content preserved:'));
